...
enum Planet1: Int, CaseIterable {
    case mercury = 1, venus, earth, mars, jupiter, saturn, uranus, neptune
}
for planetName in Planet1.allCases {
    print(planetName)
}
//: In the example above, `Planet1.mercury` has an explicit raw value of `1`, `Planet1.venus` has an implicit raw value of `2`, and so on.
//:
...
//: In the example above, `CompassPoint1.south` has an implicit raw value of "south", and so on.
//:
...
// possiblePlanet is of type Planet1? and equals Planet1.uranus
//: Not all possible `Int` values will find a matching planet, however. Because of this, the raw value initializer always returns an *optional* enumeration case. In the example above, `possiblePlanet` is of type `Planet1?`, or “optional Planet1.”
//: - NOTE:
...
//:
//: If you try to find a planet with a position of `11`, the optional `Planet1` value returned by the raw value initializer will be `nil`:
...
//: This example uses optional binding to try to access a planet with a raw value of `11`. The statement `if let somePlanet = Planet1(rawValue: 11)` creates an optional `Planet1`, and sets `somePlanet` to the value of that optional `Planet1` if it can be retrieved. In this case, it isn’t possible to retrieve a planet with a position of `11`, and so the else branch is executed instead.
//:
