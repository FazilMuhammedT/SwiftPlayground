...
// this triggers a runtime error - Fatal error: Unexpectedly found nil while unwrapping an Optional value
//:
...
//:
let john1 = Person1()
if let roomCount = john1.residence1?.numberOfRooms {
    print("John's residence has \(roomCount) room(s).")
} else {
    print("Unable to retrieve the number of rooms.")
}
...
john1.residence1?.address = someAddress
//:
...
john1.residence1?.address = createAddress()
//:
...
//:
if (john1.residence1?.address = someAddress) != nil {
...
//:
john1.residence1?[0] = Room(name: "Bathroom")
//:
...
testScores["Bev"]?[0] += 1
testScores["Brian"]?[0] = 72 //Fails because testScores doesn't contain a key for "Brian"
...
//:
if let johnsStreet = john1.residence1?.address?.street {
...
john1.residence1?.address = johnsAddress
...
//:
 if let beginsWithThe = john1.residence1?.address?.buildingIdentifier()?.hasPrefix("The") {
    if beginsWithThe {
